 ...
 *
 * AWS Bedrock Knowledge Base backend for the sagemaker-udop-processor example,
 * mirroring examples/unified-processor/knowledge-base.tf. The whole stack is
 * gated on `local.knowledge_base_enabled` (= var.create_knowledge_base, default
 * on), so setting create_knowledge_base = false drops the KB entirely.
 *
 * When enabled it stands up an OpenSearch Serverless vector collection, a
 * Bedrock Knowledge Base + S3 data source that ingests the pipeline's processed
 * OUTPUT bucket, and an ingestion Lambda. The KB ARN is wired to the root module
 * via api.knowledge_base so the Web UI's "Agent Companion Chat" / "Document KB"
 * tools can query processed documents.
 *
